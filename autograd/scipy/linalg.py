...
def grad_solve_banded(argnum, ans, l_and_u, a, b):
    ...
    def transpose_banded(l_and_u, a):
        ...
        for rr in range(1, num_rows):
            T_a = anp.vstack(
                [T_a, anp.flipud(anp.roll(a[rr:rr + 1, :], shifts[rr]))])
        T_a = anp.flipud(T_a)
    ...
    def banded_dot(l_and_u, uu, vv):
        ...
        for rr in range(1, l_and_u[0] + 1):
            banded_uv_rr = anp.hstack(
                [anp.ravel(uu)[rr:] * anp.ravel(vv)[:-rr],
                 anp.zeros(rr)])
    ...
    if argnum == 1:
        return lambda g: -banded_dot(l_and_u,
                                     updim(solve_banded(T_l_and_u, T_a, g)),
                                     anp.transpose(updim(ans)))
    elif argnum == 2:
