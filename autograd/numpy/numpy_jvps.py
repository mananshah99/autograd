...
# ----- Simple grads -----
defjvp(
    anp.abs, lambda g, ans, x: anp.real(g * replace_zero(anp.conj(x), 0.0)) /
    replace_zero(ans, 1.0))
defjvp(anp.fabs,
       lambda g, ans, x: anp.sign(x) * g)  # fabs doesn't take complex numbers.
...
defjvp(
    anp.clip, lambda g, ans, x, a_min, a_max: g * anp.logical_and(
        ans != a_min, ans != a_max))
...
defjvp(
    anp.angle,
    lambda g, ans, x: match_complex(ans,
...
defjvp(anp.prod,
       lambda g, ans, x, axis=None, keepdims=False: ans * anp.sum(
           g / x, axis=axis, keepdims=keepdims))
defjvp(
    anp.linspace,
    lambda g, ans, start, stop, *args, **kwargs: anp.linspace(
        g, 0, *args, **kwargs),
    lambda g, ans, start, stop, *args, **kwargs: anp.linspace(
        0, g, *args, **kwargs),
)
...
def forward_grad_np_var(g, ans, x, axis=None, ddof=0, keepdims=False):
    ...
    return 2.0 * anp.sum(anp.real(g * x_minus_mean),
                         axis=axis,
                         keepdims=keepdims) / (num_reps - ddof)
...
def forward_grad_np_std(g, ans, x, axis=None, ddof=0, keepdims=False):
    ...
    x_minus_mean = anp.conj(x - anp.mean(x, axis=axis, keepdims=True))
    return anp.sum(anp.real(g * x_minus_mean), axis=axis,
...
def fwd_grad_chooser(g, ans, x, axis=None, keepdims=False):
    ...
    return anp.sum(
        (g * chosen_locations), axis=axis, keepdims=keepdims) / anp.sum(
...
def broadcast(x, target):
    target_shape, target_ndim, target_dtype, target_iscomplex = anp.metadata(
        target)
...
defjvp(anp.pad,
       lambda g, ans, array, width, mode, **kwargs: anp.pad(g, width, mode))
